...
import { UserPlus, Users, History, LogOut, Copy, MessageCircle, Mail, FileDown, Calendar, Shield, CalendarSearch, Sparkles, KeyRound } from 'lucide-react'
...
export default function OfertaPage() {
  ...
  function setToateMasa(tip: 'alacarte' | 'diurna') {
    setLinii(prev => prev.map(l => ({ ...l, tipMasa: tip, diurnaPerPers: tip === 'diurna' && !l.diurnaPerPers ? 200 : l.diurnaPerPers })))
  }
  ...
  function esteLocalBucIlfov(l: Linie): boolean {
    ...
    // artistul e acasa: acelasi oras ca evenimentul, indiferent care e
    if (orasArtist && orasEv && orasArtist === orasEv) return true
    // artistul e acasa: acelasi oras ca evenimentul, indiferent care e
    if (orasArtist && orasEv && orasArtist === orasEv) return true
  ...
  function genText(): string {
    ...
    for (const l of linii.filter(x => x.includeExport)) {
      ...
      if (institutiePublica) {
        ...
        if (!c.local && c.diurnaTotal > 0) out.push('Diurna: ' + (l.diurnaFixa > 0 ? c.diurnaTotal.toLocaleString('ro-RO') + ' lei' : l.diurnaPerPers + ' lei/pers x ' + l.persoane + ' pers' + (l.zile > 1 ? ' x ' + l.zile + ' zile' : '') + ' = ' + c.diurnaTotal.toLocaleString('ro-RO') + ' lei') + ' + TVA')
        ...
        if (l.useAlcool && c.alcoolTotal > 0) out.push('Protocol: ' + c.alcoolTotal.toLocaleString('ro-RO') + ' lei (' + l.persoane + ' persoane)')
        ...
        if (!c.local && c.diurnaTotal > 0) parts.push('diurna ' + (l.diurnaFixa > 0 ? c.diurnaTotal.toLocaleString('ro-RO') + ' lei' : l.diurnaPerPers + ' lei/pers x ' + l.persoane + ' pers' + (l.zile > 1 ? ' x ' + l.zile + ' zile' : '') + ' = ' + c.diurnaTotal.toLocaleString('ro-RO') + ' lei') + ' + TVA')
  ...
  async function downloadPDF() {
    ...
    for (const l of activi) {
      ...
      if (!c.local && c.diurnaTotal > 0) rows.push('Diurna: ' + (l.diurnaFixa > 0 ? c.diurnaTotal.toLocaleString('ro-RO') + ' lei' : l.diurnaPerPers + ' lei/pers x ' + l.persoane + ' pers' + (l.zile > 1 ? ' x ' + l.zile + ' zile' : '') + ' = ' + c.diurnaTotal.toLocaleString('ro-RO') + ' lei') + ' + TVA')
      ...
      if (l.useAlcool && c.alcoolTotal > 0) rows.push('Protocol: ' + c.alcoolTotal.toLocaleString('ro-RO') + ' lei (' + l.persoane + ' persoane)')
        ...
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'24px', position:'sticky', top:'12px', zIndex:50, background:'rgba(255,255,255,0.7)', backdropFilter:'blur(16px)', WebkitBackdropFilter:'blur(16px)', border:'1px solid rgba(255,255,255,0.5)', borderRadius:'16px', padding:'12px 18px', boxShadow:'0 8px 32px rgba(0,0,0,0.06), inset 0 1px 0 rgba(255,255,255,0.5)'}}>
          <div>
            <div onClick={resetOferta} title="Ofertă nouă" style={{display:'flex', alignItems:'center', gap:'9px', cursor:'pointer'}}>
              ...
              <span style={{fontSize: isMobile ? '20px' : '27px', fontWeight:800, letterSpacing:'-1px', color:UI.ink}}>GIG<span style={{color:'#059669'}}>x</span></span>
            </div>
            {!isMobile && <div style={{fontSize:'13px', color:'#a8a29e', fontWeight:500, marginTop:'2px'}}>Generator deviz intern</div>}
          </div>
          <a href="/oferta/fisa" style={{fontSize:'13px', fontWeight:700, color:'#059669', textDecoration:'none', background:'rgba(5,150,105,0.08)', padding:'8px 12px', borderRadius:'10px', whiteSpace:'nowrap', flexShrink:0}}>Fișă →</a>
          <div style={{display:'flex', gap:'8px', alignItems:'center', flexWrap:'nowrap', overflowX:'auto', WebkitOverflowScrolling:'touch', maxWidth: isMobile ? '58vw' : 'none', paddingBottom:'2px'}}>
            ...
            <Link href="/oferta/istoric" style={{display:'flex', alignItems:'center', gap:'6px', fontSize:'13px', color:'#1c1917', fontWeight:700, background:'white', border:'1.5px solid #ececec', borderRadius:'10px', padding:'9px 12px', cursor:'pointer', fontFamily:F, textDecoration:'none', whiteSpace:'nowrap', flexShrink:0}}><History size={15} strokeWidth={2.2} />{!isMobile && ' Istoric'}</Link>
            <Link href="/oferta/roster" style={{display:'flex', alignItems:'center', gap:'6px', fontSize:'13px', color:'#1c1917', fontWeight:700, background:'white', border:'1.5px solid #ececec', borderRadius:'10px', padding:'9px 12px', cursor:'pointer', fontFamily:F, textDecoration:'none', whiteSpace:'nowrap', flexShrink:0}}><Users size={15} strokeWidth={2.2} />{!isMobile && ' Roster'}</Link>
            <button onClick={() => setShowAddArtist(true)} style={{display:'flex', alignItems:'center', gap:'6px', fontSize:'13px', color:'#1c1917', fontWeight:700, background:'white', border:'1.5px solid #ececec', borderRadius:'10px', padding:'9px 12px', cursor:'pointer', fontFamily:F, whiteSpace:'nowrap', flexShrink:0}}><UserPlus size={15} strokeWidth={2.2} />{!isMobile && ' Artist'}</button>
            {userRole === 'oferta_admin' && <Link href="/oferta/admin" style={{display:'flex', alignItems:'center', gap:'6px', fontSize:'13px', color:'#1c1917', fontWeight:700, background:'white', border:'1.5px solid #ececec', borderRadius:'10px', padding:'9px 12px', cursor:'pointer', fontFamily:F, textDecoration:'none', whiteSpace:'nowrap', flexShrink:0}}><Shield size={15} strokeWidth={2.2} />{!isMobile && ' Admin'}</Link>}
